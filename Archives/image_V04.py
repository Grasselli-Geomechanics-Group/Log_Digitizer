...
top_of_log = 764  # top of lithology bar on a 300dpi @ Pixel 793
...
def matching(match_fil_name, folder, threshold):
    ...
    print("\033[92m\033[1mProcessing %s\033[0m" % os.path.basename(folder).upper())
    ...
    for count, name in enumerate(templates_folder):
        ...
        res = cv2.matchTemplate(img_gray, template, cv2.TM_CCOEFF_NORMED)  # Match template to image using normalized correlation coefficient
        ...
        # Since the match may find more than one hit. A proximity of 2 is defined to indicate uniqueness.
        ...
        print("%s - \033[1m%s\033[0m has \033[1m%s\033[0m matches." % (os.path.basename(folder).upper(), os.path.basename(name), len(set(unique))))
    ...
    # Write csv file indicating the depth and template matched.
