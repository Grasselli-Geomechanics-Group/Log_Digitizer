...
def convert(pdf_name):
    from wand.image import Image
    from wand.color import Color
    global fil_name, resol
    resol = 300
...
def load_templates(folder):
    templates_folder =[]
    template_folder = os.path.join((os.path.dirname(folder)), 'templates')
...
def matching():
    ...
    limit = 127 # Binary Threshold Limit
    (thresh, im_bw) = cv2.threshold(img_gray, limit, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU) # Convert to binary using OTSU Threshold
    # im_bw = cv2.imread(os.path.abspath(fil_name))
    cv2.imwrite(os.path.join(os.path.dirname(fil_name), 'bw_image.png'), im_bw) # Write binary Image

    templates_folder = load_templates(fil_name) # Load Templates from Folder
    # colors = [(0, 255, 255), (0, 0, 255), (255, 0, 0)] # Box Colors
    ...
    for count, name in enumerate(templates_folder):
        ...
        template = cv2.imread(os.path.abspath(name)) # Read Template Image as RGB
        template_gray = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)  # Convert Template Image to grayscale
        limit = 127  # Binary Threshold Limit
        (thresh, temp_im_bw) = cv2.threshold(template_gray, limit, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)  # Convert Template to binary using OTSU Threshold
        w, h = temp_im_bw.shape[::-1]
        res = cv2.matchTemplate(im_bw, temp_im_bw, cv2.TM_CCOEFF_NORMED)
        # min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(res) # Returns location of bounding matching box
        threshold = 0.5
        ...
        print(os.path.basename(name), len(set(unique)))
        temp_locations, remove = [], []
    cv2.imwrite(os.path.join(os.path.dirname(fil_name), 'detected.png'), img_bgr)
