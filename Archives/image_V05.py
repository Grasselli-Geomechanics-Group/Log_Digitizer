...
def bed(approx_x, approx_y):
    ...
    if len(set(bedding_surface)) == 1:
        contact_type = "Conformity (smooth)"
    else:
        contact_type = 'Disconformity (rough)'
    return contact_type
...
DIGITIZE LOG
    ...
    # - Returns unconformity (based on color scheme along the contact of the stratigraphical sequence)
    # - Returns the flooding surface
'''
...
def digitized_log(color_map):
    ...
    with open(output_file, 'w') as writecsv:
        ...
        if legend == litho_legend:
            writer.writerow(["From (m)", "To (m)", "Color Code", "Lithofacies", "Unconformity to next Surface"])
        elif legend == surface_legend:
            writer.writerow(["From (m)", "To (m)", "Color Code", "Flooding Surface"])
